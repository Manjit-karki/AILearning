...
const FlashcardManager = ({ documentId }) => {
  ...
  const handleToggleStar = async (cardId) => {
    try {
     await flashcardService.toggleStart(cardId);
  ...
  const renderFlashcardViewer = () => {
    ...
    return (
      <div className="space-y-8">
        ...
        <button onClick={() => setSelectedSet(null)} className="group inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-emerald-600 transition-color duration-600">
        ...
        <div className="flex flex-col items-center space-y-8">
          <div className="w-full max-w-2xl">
            <Flashcard flashcard={currentCard} onToggleStart={handleToggleStar} />
          </div>
      ...
      return (
        <div className="flex flex-col items-center justify-center py-16 px-6">
          ...
          <button onClick={handleGenerateFlashcards} disabled={generating} className="group inline-flex items-center gap-2 px-6 h-12 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100">
            {generating ? (
              <>
                <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spiral" />
                Generating...
              </>
            ) : (
              <>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleGenerateFlashcards} disabled={generating} className="group inline-flex items-center gap-2 px-5 h-11 bg-linear-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 font-semibold text-sm rounded-xl transition-all duration-200 shadow=lg shadow-emerald-500/25 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100">
            {generating ? (
              <>
                <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animation-spin" />
                Generating...
              </>
            ) : (
              <>
